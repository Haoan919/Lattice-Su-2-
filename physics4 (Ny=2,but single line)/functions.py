...
def WilsonAction(nu, mu, R, T, U, Nt, Nx):#这个是单格子的
    """Name says"""
    if mu <= nu: raise Exception("mu<=nu")
    if mu == 2 and T > 1: raise Exception("T>1")
            ...
            for y in range(Ny):
                ...
                for j in range(R):
                    loop = np.dot(
                        loop,
                        U[
                            (t + T * a_mu[0] + j * a_nu[0]) % Nt,
                            (x + T * a_mu[1] + j * a_nu[1]) % Nx,
                            (y + T * a_mu[2] + j * a_nu[2]) % Ny,
                            nu,
                        ],
                    )
                ...
                for i in range(T - 1, -1, -1):
                    loop = np.dot(
                        loop,
                        U[
                            (t + i * a_mu[0] + R * a_nu[0]) % Nt,
                            (x + i * a_mu[1] + R * a_nu[1]) % Nx,
                            (y + i * a_mu[2] + R * a_nu[2]) % Ny,
                            mu,
                        ]
                        .conj()
                        .T,
...
@njit()
def staple_calculus(t, x, y, mu, U, Nt, Nx):
